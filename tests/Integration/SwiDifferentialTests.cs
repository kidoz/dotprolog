...
public sealed class SwiDifferentialTests
{
    ...
    public static readonly TheoryData<string> Corpus = new(
        ...
        "set_prolog_flag(occurs_check, error), catch(X = f(X), error(_, _), write(caught))",
        ...
        "catch(string_concat([a, b], [c], _), error(E, _), true), writeq(E)",
        "catch(string_concat(_, f(x), ab), error(E, _), true), writeq(E)",
        "catch(string_concat(ab, _, _), error(E, _), true), writeq(E)",
        "atom_chars(abc, L), string_length(L, N), write(N)",
        "atom_codes(abc, L), string_length(L, N), write(N)",
        "catch(string_length([a|_], _), error(E, _), true), writeq(E)",
        "catch(string_length([a, 98], _), error(E, _), true), writeq(E)",
        "catch(string_length([97, b], _), error(E, _), true), writeq(E)",
        "catch(string_length([foo], _), error(E, _), true), writeq(E)",
        "catch(string_length(f(x), _), error(E, _), true), writeq(E)",
        "atom_chars('a,b', L), split_string(L, ',', '', P), writeq(P)",
        "catch(split_string(42, ',', '', _), error(E, _), true), writeq(E)",
        "atom_codes(abc, L), string_code(2, L, C), write(C)",
        "catch(string_code(-1, abc, _), error(E, _), true), writeq(E)",
        "atom_chars('42', L), number_string(N, L), write(N)",
        "catch(number_string(_, abc), error(E, _), true), writeq(E)",
        "atom_chars(' 42', L), ( number_string(_, L) -> write(yes) ; write(no) )",
        "atom_chars(abc, L), ( atom_string(abc, L) -> write(yes) ; write(no) )",
        "atom_chars(xy, L), text_to_string(L, S), writeq(S)",
        "( string_upper(abc, 'ABC') -> write(yes) ; write(no) )",
        "catch(string_upper(abc, f(x)), error(E, _), true), writeq(E)",
        "atom_string(ab, S), upcase_atom(S, U), writeq(U)",
        "atom_string(ab, S), atomic_list_concat([S, c], R), writeq(R)",
        "catch(atomic_list_concat([[a], b], _), error(E, _), true), writeq(E)",
        "catch(sub_string([a], 0, 1, _, _), error(E, _), true), writeq(E)",
        "atom_chars('f(a)', L), term_to_atom(T, L), writeq(T)",
        "term_to_atom([a, b], A), writeq(A)",
        "term_to_atom(1+2, A), writeq(A)",
        "atom_codes('g(1)', L), read_term_from_atom(L, T, []), writeq(T)",
        "catch(read_term_from_atom(42, _, []), error(E, _), true), writeq(E)",
        "with_output_to(codes(C, [0'!]), write(hi)), atom_codes(A, C), writeq(A)",
        "with_output_to(chars(C, [!]), write(hi)), atom_chars(A, C), writeq(A)",
        "catch(with_output_to(foo(x), true), error(E, _), true), writeq(E)",
        "catch(with_output_to(_, true), error(E, _), true), writeq(E)",
        "catch(format(42, '~w', [x]), error(E, _), true), writeq(E)",
        "catch(format(atom(_), '~a', [42]), error(E, _), true), writeq(E)",
        "atom_chars(abc, L), format(atom(A), '~s', [L]), writeq(A)",
        "atom_string(ab, S), format(atom(A), '~a', [S]), writeq(A)",
        "catch(must_be(text, [a|_]), error(E, _), true), writeq(E)",
        "char_code(E, 128512), atom_concat(a, E, X), atom_concat(X, b, T), atom_length(T, N), write(N)",
        "char_code(E, 128512), atom_concat(a, E, X), atom_codes(X, L), write(L)",
        "char_code(E, 128512), atom_concat(a, E, X), atom_concat(X, b, T), findall(B-L, sub_atom(T, B, L, _, _), All), length(All, N), write(N)",
        "char_code(E, 128512), atom_concat(a, E, X), atom_concat(X, b, T), sub_atom(T, B, 1, 0, b), write(B)",
        "char_code(E, 128512), atom_concat(a, E, X), findall(P, atom_concat(P, _, X), All), length(All, N), write(N)",
        "char_code(D, 66600), upcase_atom(D, U), atom_codes(U, L), write(L)",
        "char_code(E, 128512), atom_concat('0\\'', E, T), atom_number(T, N), write(N)",
        "char_code(E, 128512), atom_string(E, S), string_length(S, N), write(N)",
        "char_code(E, 128512), atom_concat(a, E, X), atom_concat(X, b, T), string_code(2, T, C), write(C)",
        "char_code(E, 128512), atom_concat(a, E, X), atom_concat(X, b, T), split_string(T, E, '', P), length(P, N), write(N)",
        "char_code(E, 128512), atom_concat(a, E, X), atom_concat(X, b, T), sub_string(T, 1, 1, A, S), string_code(1, S, C), write(A-C)",
        "char_code(D, 66560), ( char_type(D, upper(L)) -> char_code(L, C), write(C) ; write(none) )",
        "catch(code_type(1114112, _), error(E, _), true), write(E)",
        "atom_codes(A, [128512, 97]), atom_chars(A, Cs), length(Cs, N), write(N)",
        "X = 0'😀, write(X)",
        "atom_codes('\\x1F600\\', L), write(L)",
        "atom_codes('\\U0001F600\\u00e9', L), write(L)",
        "X = 'a😀b', atom_length(X, N), write(N)",
        "X = 𝑎bc, atom_length(X, N), write(N)",
        "writeq(f('𝑎bc', 'a😀b', 𐐨x))",
        "term_to_atom(T, 'g(𝑎bc)'), T = g(A), atom_length(A, N), write(N)",
        "format(atom(A), '~c~c', [128512, 97]), atom_codes(A, L), write(L)",
        "catch(format(atom(_), '~c', [1114112]), error(E, _), true), write(E)",
        "catch(format(atom(_), '~c', [-1]), error(E, _), true), write(E)",
        "char_code(E, 128512), format(atom(A), '~w~t~6|x', [E]), atom_length(A, N), write(N)",
        "char_code(E, 128512), format(atom(A), '~a~t~4+~a', [E, z]), atom_length(A, N), write(N)",
        "char_code(E, 128512), with_output_to(codes(C), (put_char(E), put_code(97))), write(C)",
        "msort(['\\x1F600\\', '\\xFFFD\\', a, '\\xE000\\'], L), maplist(char_code, L, C), write(C)",
        "char_code(E, 128512), char_code(R, 65533), atom_string(E, SE), atom_string(R, SR), compare(O, SE, SR), write(O)",
        "char_code(E, 128512), char_code(R, 57344), atom_concat(a, E, X), atom_concat(a, R, Y), T =.. [X, 1], U =.. [Y, 1], compare(O, T, U), write(O)",
        "char_code(E, 128512), char_code(R, 65533), setof(X, member(X, [E, R, E]), L), maplist(char_code, L, C), write(C)",
        "X = f(😀), arg(1, X, A), atom_codes(A, C), write(C)",
        "X = [€, ∀], writeq(X)",
        "writeq(- '😀')",
        "writeq('中文')",
        "atom_codes(A, [0'x, 0x301]), writeq(A)",
        "atom_codes(A, [0'a, 0x2028, 0'b]), writeq(A)",
        ...
        "catch((L = ['1'|L], number_chars(_, L)), error(type_error(list, C), _), true), nonvar(C), C = ['1'|T], C == T, write(caught)",
        ...
        "freeze(X, write(w)), X = 1, ( write(a), fail ; write(b) )",
        "( freeze(X, fail), X = 1 -> write(yes) ; write(no) )",
        "findall(Z, (freeze(X, member(Z, [a, b])), X = go), L), write(L)",
        "freeze(X, write(x)), freeze(Y, write(y)), X = Y, X = 1",
        "catch((freeze(X, throw(bad)), X = 1), bad, write(caught))",
        "freeze(X, (Y = 1)), freeze(Y, write(y)), X = 0, write(end)",
        "frozen(_, G), write(G)",
        "nth0(1, [a, b, c], E, R), write(E-R)",
        "findall(N-E-R, nth1(N, [a, b], E, R), L), write(L)",
        "call_nth(member(X, [a, b, c]), 2), write(X)",
        "findall(N, call_nth(member(_, [a, b, c]), N), L), write(L)",
        "catch(call_nth(true, foo), error(E, _), true), write(E)",
        "catch(succ(-1, _), error(E, _), true), write(E)",
        "catch(succ(_, -1), error(E, _), true), write(E)",
        "catch(nth0(foo, [a], _), error(E, _), true), write(E)",
        "catch(length(L, -1), error(E, _), true), write(E)",
        "catch(length(a, -1), error(E, _), true), write(E)",
        "catch(length(L, 1.0), error(E, _), true), write(E)",
        "catch(length([], 0+0), error(E, _), true), write(E)",
