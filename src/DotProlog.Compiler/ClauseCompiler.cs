...
internal sealed class ClauseCompiler
{
    ...
    internal int Compile(SyntaxTerm head, SyntaxTerm? body)
    {
        ...
        body = body is null
            ? null
            : TermNormalizer.Normalize(body, _program.Flags.DoubleQuotes, _program.Flags.CodePointCharacters);
    ...
    private bool IsStrictIsoExtension(int functorId, string name, int arity)
    {
        if (
            ...
            || (name == "$write_term" && arity is 3 or 4)
            || (name == "$read" && arity is 2 or 3)
            || (name == "$read_term" && arity is 3 or 4)
