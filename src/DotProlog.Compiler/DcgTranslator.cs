    ...
    private bool TryTranslateBody(SyntaxTerm element, SyntaxTerm start, SyntaxTerm end, out SyntaxTerm goal)
    {
        ...
        switch (element)
        {
            ...
            case StringTerm text:
                return TryTranslateBody(TermNormalizer.Normalize(text, codePoints: !_strictIso), start, end, out goal);
            ...
            case StringValueTerm text:
                return TryTranslateBody(
                    TermNormalizer.Normalize(new StringTerm(text.Value, text.Span), codePoints: !_strictIso),
                    start,
                    end,
                    out goal
                );
            ...
            case CompoundTerm { Name: "phrase", Arity: 1 } phrase:
                goal = new CompoundTerm("phrase", [.. phrase.Arguments, start, end], element.Span);
                return true;
