...
| Unbounded integers and rationals (GMP) | Integers are supported unbounded: 60-bit tagged fixnums promote to interned big integers on overflow, literals of any length read exactly, and `bounded` is `false`, differential-verified against SWI for arithmetic, literals, ordering, indexing, and `format/2`. Interned big integers live for the program's lifetime, an embedding consideration beside atom growth. Divergences: an absurd shift count or exponent raises `resource_error(memory)` where SWI names its stack, and `^` with a negative integer exponent keeps ISO's `type_error(float, Base)` where SWI answers a float or rational. Rationals are the remaining part of this phase |
